...
function Page() {
  ...
  const save = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        unlocks_per_job: s.unlocks_per_job,
        db_searches_per_hour: s.db_searches_per_hour,
        db_rows_per_day: s.db_rows_per_day,
      })
      .eq("id", 1);
    setSaving(false);
  ...
  if (!s)
    return (
      <AdminShell title="Plan Settings">
  ...
  return (
    <AdminShell
      title="Plan & Free-tier Settings"
      subtitle="Global rules for free posts, WhatsApp caps, unlock cost, and spam thresholds."
      actions={
        <button
          onClick={save}
          disabled={saving}
    ...
    >
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="space-y-3 rounded-2xl border border-border bg-card p-5">
          ...
          {bool(
            "free_whatsapp_rajasthan_only",
            "WhatsApp alerts: Rajasthan only",
            "If on, free WA alerts only send to Rajasthan candidates.",
          )}
        </section>
        <section className="space-y-3 rounded-2xl border border-border bg-card p-5">
          ...
          {num(
            "credits_per_unlock",
            "Credits per candidate unlock",
            "Charged once a job's own unlock allowance runs out.",
          )}
          {num(
            "custom_plan_min_amount",
            "Custom plan unlocks above (₹)",
            "Employers who spend this much can request a custom plan.",
          )}
          ...
          {num(
            ...
            "Searches per company per hour",
            "Anti-scraping rate limit on search_candidates_for_company.",
          )}
          {num(
            "db_rows_per_day",
            "Result rows per company per day",
            "Anti-scraping cap on cumulative rows returned by DB search.",
          )}
          <h3 className="pt-4 text-sm font-bold">Anti-spam</h3>
          {num(
            "spam_jobs_per_hour",
            "Spam threshold (jobs/hour)",
            "Auto-flag accounts above this rate.",
          )}
        </section>
      </div>
    </AdminShell>
