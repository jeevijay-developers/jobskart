...
/* ---------- Recommended jobs pagination (same pattern as src/routes/jobs.tsx) ---------- */

function RecJobsPagination({
  page,
  totalPages,
  onChange,
}: {
  ...
  return (
    <nav className="mt-6 flex flex-wrap items-center justify-center gap-1" aria-label="Recommended jobs pagination">
      <button
        type="button"
      ...
      {start > 1 && (
        <>
          <RecPageBtn n={1} active={page === 1} onClick={onChange} />
          {start > 2 && <span className="px-2 text-muted-foreground">…</span>}
        </>
      )}
      {pages.map((p) => (
        <RecPageBtn key={p} n={p} active={p === page} onClick={onChange} />
      ))}
      {end < totalPages && (
        <>
          {end < totalPages - 1 && <span className="px-2 text-muted-foreground">…</span>}
          <RecPageBtn n={totalPages} active={page === totalPages} onClick={onChange} />
        </>
      )}
      <button
        type="button"
...
function RecPageBtn({ n, active, onClick }: { n: number; active: boolean; onClick: (p: number) => void }) {
  return (
    <button
      type="button"
      onClick={() => onClick(n)}
