...
export function JobWizard({ editJobId }: { editJobId?: string }) {
    ...
    (async () => {
      try {
        ...
        if (editJobId) {
          ...
          setForm(jobToForm(job as unknown as JobRow));
        }
      } catch (e) {
  ...
  // Regenerate JD when arriving at step 3 if empty (an edited job already has
  // description_html from the DB, so this only fires for brand-new jobs).
  useEffect(() => {
  ...
  // Edit mode never re-charges tier cost or touches status — it only ever
  // updates the fields a "Post a job" submission would otherwise set.
  ...
  return (
    <EmployerShell
      ...
      subtitle={editJobId ? "Update any field, then save your changes." : "4 steps. Takes about 3 minutes."}
    >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {editJobId ? (
                    <>
                      ...
                      <button type="button" onClick={saveEdit} disabled={saving || !form.title.trim()}
